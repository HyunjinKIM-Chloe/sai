...
class Preprocessing(db.Query):
    ...
    def read_file(self, url):
        remote_path = f"https://data.eihba.com/track/{url}"
...
class MakeFreqDataset(Preprocessing):
    ...
    def make_frequency_df(self, start=None, end=None):
        ls_chroma_stft_mean = []
        ls_chroma_stft_var = []
        ls_rms_mean = []
        ls_rms_var = []
        ...
        for uri in tqdm(tracks_df['uri'].tolist()[start:end]):
            y, sr = self.read_file(uri)
        ...
        result_df = pd.DataFrame({
            "track_id": tracks_df['id'].tolist()[start:end],
