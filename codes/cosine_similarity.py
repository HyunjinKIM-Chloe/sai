...
class CosineSimilarity():
    ...
    def make_cos_df(self, freq_df, z_scale=True):
        if z_scale:
            input_df = (freq_df - np.mean(freq_df)) / np.std(freq_df)
        else:
        ...
        cos_df.set_index(freq_df.index, inplace=True)
        cos_df.columns = freq_df.index
        return cos_df
